...
Model.prototype = {
	...
	save: function(instance, callback) {
		...
		var doc = instance.toVO();
		var validationError = validation(doc,this._constraints);
		if(!validationError){
			if (callback) this._db.insert(doc, callback);
			else return Q.ninvoke(this._db, 'insert', doc);
		} else {
            var error = new Error("Model Validation Error");
